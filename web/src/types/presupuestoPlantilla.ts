export type PresupuestoPlantillaConfig = {
  body: string;
  /** Plantilla PDF para pacientes externos (valor único, sin 3 cuotas). */
  bodyExterno: string;
};
...
export const DEFAULT_PRESUPUESTO_PLANTILLA_BODY_EXTERNO =
  `<div>Estimado/a {{nombrePaciente}},</div>` +
  `<br>` +
  `<div>Por medio de la presente se describe el módulo de evaluación según indicación de {{nombreProfesional}}.</div>` +
  `<br>` +
...
export const PRESUPUESTO_PLANTILLA_VAR_LABELS: Record<PresupuestoPlantillaVar, string> = {
...
export type PresupuestoPlantillaKind = "normal" | "externo";

export const EMPTY_PRESUPUESTO_PLANTILLA_CONFIG: PresupuestoPlantillaConfig = {
  body: DEFAULT_PRESUPUESTO_PLANTILLA_BODY,
  bodyExterno: DEFAULT_PRESUPUESTO_PLANTILLA_BODY_EXTERNO,
};
