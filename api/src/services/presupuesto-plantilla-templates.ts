...
export function presupuestoPlantillaConfigWithDefaults(
  ...
  const storedBodyExterno = stored?.bodyExterno?.trim() || "";
  return {
    body: migratePlantillaBodyLugar(storedBody || DEFAULT_PRESUPUESTO_PLANTILLA_CONFIG.body),
    bodyExterno: migratePlantillaBodyLugar(
      storedBodyExterno || DEFAULT_PRESUPUESTO_PLANTILLA_CONFIG.bodyExterno,
    ),
  };
}
